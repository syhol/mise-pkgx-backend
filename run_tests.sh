...
run_unit_tests() {
    ...
    if lua test_platform_detection.lua; then
        log "✓ Platform detection tests passed"
    else
        error "✗ Platform detection tests failed"
        failed=1
    fi
    echo
    ...
    if lua test_version_listing.lua; then
        log "✓ Version listing tests passed"
    else
        error "✗ Version listing tests failed"
        failed=1
    fi
    echo
    ...
    if lua test_exec_env.lua; then
        log "✓ Exec environment tests passed"
    else
        error "✗ Exec environment tests failed"
        failed=1
    fi
    echo
...
run_integration_tests() {
    ...
    if ./test_basic_integration.sh; then
        log "✓ Basic integration tests passed"
        return 0
    else
        error "✗ Basic integration tests failed"
        return 1
    fi
}
